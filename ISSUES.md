...
- Secondary nav link styling is inconsistent between `templates/account.html` and `templates/index.html`, with one using a neutral style for "← Back to Dashboard" (line 319-323) and the other giving it an indigo primary treatment (line 311-21). Decide on the correct secondary-link style. Two different "key stats" layouts are used for the same content: `templates/account.html` has a borderless flex row (`line 353-359`) while `templates/index.html` uses a bordered grid with hairline dividers (`line 261-268`). Choose one layout. Decorative emoji in headings on `templates/index.html` (📚/🚀/📁, lines 427/451/458) should be decided whether they are part of the visual language or dropped from `templates/account.html`.
